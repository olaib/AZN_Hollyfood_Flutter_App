...
import 'package:azn_hollyfood_flutter_app/exceptions/custom_exceptions.dart';
...
class AuthenticationProvider with ChangeNotifier {
  ...
  AuthenticationProvider() {
    ...
    _gsheets = GetIt.instance.get<GSheetsProvider>();
    ...
    _firebaseAuth.authStateChanges().listen((User? user) async {
      if (_user != null) {
        ...
        await _db.getUserByUsername(_user!.username).then((user) {
          _user = user;
          // final Map<String, dynamic> userData =
          //     snapshot.docs.first.data() as Map<String, dynamic>;
          // userData["uid"] = _user!.uid;
          // _user = UserEntity.fromJson(userData);
          _nav.removeAndNavigateTo(
              (_user!.isAdmin) ? Routes.adminPanel : Routes.home);
        });
      } else if (_nav.currentRoute != Routes.initialRoute.url) {
        _nav.removeAndNavigateTo(Routes.initialRoute);
      }
    });
  }
      ...
      await (isAdmin ? _db.getAdmin(username) : _db.getUserByUsername(username))
        ...
        if (user != null) {
          if (user.password == password) {
            ...
            await _gsheets.updateLogin(user);
  ...
  Future<void> signInWithEmailAndPassword(
    ...
    try {
      Log.debug('loging as App manager: email: $email, password: $password');
      await _firebaseAuth.signInWithEmailAndPassword(
          email: email, password: password);
      Log.info('signed in Successfully');
    } on FirebaseAuthException catch (e) {
      Log.error('login Error: ${e.code}');
      throw LoginException(FIREBASE_AUTH_MAP[e.code] ?? UNKNOWN_ERROR);
    } catch (e) {
      Log.error('login Error: $e');
      throw LoginException(e.toString());
